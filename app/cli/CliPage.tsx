...
.cli-code-body {
  ...
  font-size: 13px;
  line-height: 1.7;
  color: #cdd6f4;        /* Catppuccin Mocha text — soft white, not harsh */
}

/* Comments like # 1. Start the tunnel */
.cli-code-body .comment { color: #6c7086; }

/* $ prompt symbol */
.cli-code-body .prompt  { color: #89b4fa; }

/* Command name — npx */
.cli-code-body .cmd     { color: #cdd6f4; }

/* Package name — @hookflo/tern-dev */
.cli-code-body .pkg     { color: #a6e3a1; }

/* Flags — --port */
.cli-code-body .flag    { color: #f38ba8; }

/* Numbers — 3000 */
.cli-code-body .num     { color: #fab387; }

/* ASCII art logo */
.cli-code-body .logo    { color: #cba6f7; }

/* URLs — tunnel/dashboard values */
.cli-code-body .url     { color: #89dceb; }

/* Labels — tunnel, dashboard, forwarding */
.cli-code-body .label   { color: #6c7086; }

/* Success checkmark line */
.cli-code-body .success { color: #a6e3a1; }

.cli-code-body {
  font-family: 'Noto Sans Mono', 'Courier New', ui-monospace, monospace;
}
.cli-code-body .logo    { color: #2DD884; }
.cli-code-body .success { color: #2DD884; }
.cli-code-body .url     { color: #2DD884; }
.cli-code-body .pkg     { color: #2DD884; }
.cli-code-body .prompt  { color: #2DD884; }
...
/* Slightly muted for labels/arrows */
.cli-code-body .label   { color: #6c7086; }
...
useEffect(() => {
  const delays = [
    0,    // cli-line-0  — comment
    300,  // cli-line-1  — $ npx command
    650,  // cli-line-2  — logo row 1
    750,  // cli-line-3  — logo row 2
    850,  // cli-line-4  — logo row 3
    950,  // cli-line-5  — logo row 4
    1050, // cli-line-6  — logo row 5
    1150, // cli-line-7  — logo row 6
    1350, // cli-line-8  — tunnel URL
    1500, // cli-line-9  — dashboard
    1650, // cli-line-10 — forwarding
    1850, // cli-line-11 — step 2 comment
    2000, // cli-line-12 — step 3 comment
    2300, // cli-line-13 — ✓ connected
  ];

  delays.forEach((delay, i) => {
    setTimeout(() => {
      document.getElementById(`cli-line-${i}`)?.classList.add("show");
    }, delay);
  });
}, []);
  ...
  <div className="cli-code-body">
    ...
    <span className="cli-line" id="cli-line-0">
      <span className="comment"># 1. Start the tunnel{"\n"}</span>
    </span>

    <span className="cli-line" id="cli-line-1">
      <span className="prompt">$ </span>
      <span className="cmd">npx </span>
      <span className="pkg">@hookflo/tern-dev </span>
      <span className="flag">--port </span>
      <span className="num">3000</span>
      {"\n\n"}
    </span>

    <span className="cli-line" id="cli-line-2">
      <span className="logo">{"  ████████╗███████╗██████╗  ███╗  ██╗\n"}</span>
    </span>
    <span className="cli-line" id="cli-line-3">
      <span className="logo">{"     ██║   ██╔════╝██╔══██╗ ████╗ ██║\n"}</span>
    </span>
    <span className="cli-line" id="cli-line-4">
      <span className="logo">{"     ██║   █████╗  ██████╔╝ ██╔██╗██║\n"}</span>
    </span>
    <span className="cli-line" id="cli-line-5">
      <span className="logo">{"     ██║   ██╔══╝  ██╔══██╗ ██║╚████║\n"}</span>
    </span>
    <span className="cli-line" id="cli-line-6">
      <span className="logo">{"     ██║   ███████╗██║  ██║ ██║ ╚███║\n"}</span>
    </span>
    <span className="cli-line" id="cli-line-7">
      <span className="logo">{"     ╚═╝   ╚══════╝╚═╝  ╚═╝ ╚═╝  ╚══╝\n\n"}</span>
    </span>

    <span className="cli-line" id="cli-line-8">
      <span className="label">{"  tunnel    "}</span>
      <span className="comment">{"→  "}</span>
      <span className="url">https://abc123.relay.tern.hookflo.com</span>
      {"\n"}
    </span>

    <span className="cli-line" id="cli-line-9">
      <span className="label">{"  dashboard "}</span>
      <span className="comment">{"→  "}</span>
      <span className="url">http://localhost:2019</span>
      {"\n"}
    </span>

    <span className="cli-line" id="cli-line-10">
      <span className="label">{"  forwarding"}</span>
      <span className="comment">{"→  "}</span>
      <span className="url">localhost:3000</span>
      {"\n\n"}
    </span>

    <span className="cli-line" id="cli-line-11">
      <span className="comment"># 2. Copy tunnel URL to your webhook provider{"\n"}</span>
    </span>

    <span className="cli-line" id="cli-line-12">
      <span className="comment"># 3. Open dashboard and watch events flow in real-time{"\n\n"}</span>
    </span>

    <span className="cli-line" id="cli-line-13">
      <span className="success">✓ </span>
      <span className="cmd">Tunnel live </span>
      <span className="comment">· Ctrl+C to end & erase all data</span>
